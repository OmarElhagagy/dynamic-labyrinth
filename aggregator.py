...
class Aggregator:
    ...
    def get_count_in_window(self, session_id: str, rule_id: str, window_sec: int, current_time: datetime) -> int:
        """احسب عدد الأحداث خلال النافذة الزمنية"""
    ...
    def cleanup_old_events(self, older_than_hours: int = 24):
        """تنظيف السجلات القديمة لتوفير المساحة"""
        cutoff = datetime.now() - timedelta(hours=older_than_hours)
        self.conn.execute(
            "DELETE FROM aggregated_counts WHERE event_time < ?",
            (cutoff,)
        )
        self.conn.commit()
    ...
    def update_skill_score(self, session_id: str, skill_delta: int):
        """زيادة أو تقليل skill_score"""
        current = self.get_skill_score(session_id)
        new_score = max(0, current + skill_delta)  # لا يقل عن صفر
        self.conn.execute(
