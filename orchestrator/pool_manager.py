...
from models import (
    ContainerState, SessionState, ContainerLevel,
    PoolStatus, ContainerRecord, SessionRecord
)
from database import (
    ContainerModel, SessionModel, DecisionLogModel,
    get_idle_containers_by_level, get_container_by_id, get_session_by_id
)
from config import get_settings, get_pool_config, PoolConfig
...
class PoolManager:
    ...
    async def initialize_pools(self, db: AsyncSession) -> None:
        ...
        for container_def in containers:
            ...
            db.add(container)
            log.info("Created container", container_id=container_def["id"], level=container_def["level"])
    ...
    async def assign_container(
        ...
        async with self._lock:
            ...
            if not container:
                log.warning("No available containers", session_id=session_id, target_level=target_level)
                return None
            ...
            session.expires_at = datetime.utcnow() + timedelta(seconds=self.settings.session_ttl_seconds)
    ...
    async def _find_idle_container(self, db: AsyncSession, level: int) -> Optional[ContainerModel]:
    ...
    async def get_pool_status(self, db: AsyncSession) -> List[PoolStatus]:
        ...
        for level in [1, 2, 3]:
            ...
            unhealthy_result = await db.execute(
                select(func.count(ContainerModel.id)).where(
                    ContainerModel.level == level,
                    ContainerModel.healthy == False
                )
            )
            unhealthy = unhealthy_result.scalar() or 0
            
            statuses.append(PoolStatus(
                level=level,
                total=total,
                idle=idle,
                assigned=assigned,
    ...
    async def log_decision(
        ...
        action: str,
        rule_id: str,
        skill_score_before: Optional[int],
