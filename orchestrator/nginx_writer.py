...
class NginxWriter:
    ...
    async def write_map_file(self, db: AsyncSession, map_path: str | None = None) -> bool:
        """
    ...
    async def add_session_mapping(
        self, db: AsyncSession, session_id: str, session_cookie: str, container_address: str
    ) -> bool:
        """
        ...
        try:
            ...
            result = await db.execute(
                select(NginxMapEntryModel).where(NginxMapEntryModel.session_id == session_id)
            )
    ...
    async def remove_session_mapping(self, db: AsyncSession, session_id: str) -> bool:
        """
        ...
        try:
            ...
            await db.execute(
                delete(NginxMapEntryModel).where(NginxMapEntryModel.session_id == session_id)
            )
            await db.commit()
...
_nginx_writer: NginxWriter | None = None
